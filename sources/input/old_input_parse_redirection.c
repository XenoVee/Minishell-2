...
/*                                                   +#+                      */
/*   Created: 2024/07/23 14:54:41 by rmaes         #+#    #+#                 */
/*   Updated: 2024/07/26 14:01:14 by rmaes         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	parse_output_redirection(t_commands *cmds, int i)
{
	...
	{
		cmds[i].fd_output
			= malloc(sizeof(int) * (ft_count_word(cmds[i].str, '>')));
		split_result = ft_split(cmds[i].str, '>');
		j = 1;
		while (split_result[j])
		{
			remove_white_spaces_and_tabs(split_result[j], '\0', 1);
			cmds[i].fd_output[j - 1]
				= open(split_result[j], O_RDWR | O_CREAT, 00666);
			j++;
		}
